...
export async function waitForAgentRegistration(
  ...
  // Parse the AgentRegistered event log using viem's decodeEventLog and the ABI
  for (const log of receipt.logs) {
    try {
      // decodeEventLog throws if the log doesn't match the provided ABI/event
      ...
      // Depending on viem version the event name may be on `eventName` or `name`
      const eventName = decoded?.eventName ?? decoded?.name ?? null;
      if (eventName === "AgentRegistered") {
        // AgentRegistered typically has (uint256 agentId, address owner, string agentURI)
        const agentIdRaw = decoded.args?.[0] ?? decoded.args?.agentId ?? "0";
        const agentId = Number(agentIdRaw ?? 0);
        const agentUri = decoded.args?.agentURI ?? decoded.args?.agentUri ?? "";
        return {
          txHash,
          agentId,
          agentUri,
