...
interface IncidentSlippageProtection {
  enabled: boolean;
  maxSlippageBps: number;
}

type IncidentSpendRequest = SpendRequest & {
  slippageBps?: number;
};

type IncidentVCRConstraints = VCRConstraints & {
  slippageProtection?: IncidentSlippageProtection;
};

type IncidentVCRPolicy = Omit<VCRPolicy, "constraints"> & {
  constraints: IncidentVCRConstraints;
};
...
export interface IncidentReplay {
  ...
  vcrFields: string[];
  summary: string;
  x402Request: IncidentSpendRequest;
  protectedPolicy: {
...
function detectTriggeredGuards(
  policy: IncidentVCRPolicy,
  request: IncidentSpendRequest,
  dailySpent: string,
): string[] {
  const guards: string[] = [];
...
function buildProtectedPolicy(
  ...
  overrides: Partial<IncidentVCRConstraints>,
): IncidentVCRPolicy {
  const nextPolicy = clonePolicy(basePolicy) as IncidentVCRPolicy;
