...
function validateArgs(args) {
    ...
    if (args.recipients.length === 0) {
        errors.push(`No --recipient addresses provided.\n` +
    ...
    if (args.hours) {
        ...
        if (start < 0 || start > 23)
            errors.push(`--hours start (${start}) must be in range [0, 23]`);
        if (end < 1 || end > 24)
            errors.push(`--hours end (${end}) must be in range [1, 24]`);
        if (start >= end)
            errors.push(`--hours start (${start}) must be before end (${end})`);
    }
    if (errors.length > 0) {
...
// ─── Summary printer ──────────────────────────────────────────────────────────
function printSummary(args, env) {
