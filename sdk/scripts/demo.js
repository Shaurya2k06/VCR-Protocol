...
import { resolveAgentPolicy, canAgentSpend, getSpendSummary, getDailySpent, recordSpend, resetDailySpend, } from "../src/index.js";
// ─── Config ───────────────────────────────────────────────────────────────────
...
let agentRecord;
try {
    const { default: record } = await import("../agents/researcher-001.json", { assert: { type: "json" } });
    agentRecord = record;
}
catch {
    console.error("❌  agents/researcher-001.json not found.\n" +
...
async function main() {
    ...
    let policy;
    try {
        const resolved = await resolveAgentPolicy(ensName, publicClient);
        if (!resolved) {
        ...
        console.error(`│  ❌  Failed to resolve policy: ${err.message}`);
        process.exit(1);
    }
    console.log("└────────────────────────────────────────────────────────────┘\n");
    ...
    {
        const summary = await getSpendSummary(ensName, "USDC", policy.constraints.dailyLimit.amount);
